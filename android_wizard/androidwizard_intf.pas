    ...
    begin
      ...
      with TStringList.Create do
        try
          ...
          if FileExists(FPathToJavaTemplates + DirectorySeparator +
            'gdx' + DirectorySeparator + 'App.java') then
          begin
            LoadFromFile(FPathToJavaTemplates + DirectorySeparator +
              'gdx' + DirectorySeparator + 'App.java');
            Strings[0] := 'package ' + strPackName + ';'; //replace dummy App.java
            SaveToFile(FFullJavaSrcPath + DirectorySeparator + 'App.java');
          end;

          if FileExists(FPathToJavaTemplates + DirectorySeparator +
            'gdx' + DirectorySeparator + 'MyGdxGame.java') then
          begin
            LoadFromFile(FPathToJavaTemplates + DirectorySeparator +
              'gdx' + DirectorySeparator + 'MyGdxGame.java');
            Strings[0] := 'package ' + strPackName + ';'; //replace dummy
            SaveToFile(FFullJavaSrcPath + DirectorySeparator + 'MyGdxGame.java');
          end;

          if FileExists(FPathToJavaTemplates + DirectorySeparator +
            'gdx' + DirectorySeparator + 'jGdxForm.java') then
          begin
            LoadFromFile(FPathToJavaTemplates + DirectorySeparator +
              'gdx' + DirectorySeparator + 'jGdxForm.java');
            Strings[0] := 'package ' + strPackName + ';'; //replace dummy
            SaveToFile(FFullJavaSrcPath + DirectorySeparator + 'jGdxForm.java');

            ControlsJava := TStringList.Create;
            ControlsJava.LoadFromFile(FFullJavaSrcPath + DirectorySeparator +
              'Controls.java');

            LoadFromFile(FPathToJavaTemplates + DirectorySeparator +
              'gdx' + DirectorySeparator + 'jGdxForm.create');
            auxList := TStringList.Create;
            auxList.LoadFromFile(FPathToJavaTemplates + DirectorySeparator +
            ...
            ControlsJava.SaveToFile(FFullJavaSrcPath + DirectorySeparator +
              'Controls.java');
            ControlsJava.Free;
            auxList.Free;
          end;
          ...
          if FileExists(FPathToJavaTemplates + DirectorySeparator +
            'Controls.native') then
          begin
            CopyFile(FPathToJavaTemplates + DirectorySeparator + 'Controls.native',
              FAndroidProjectName + DirectorySeparator + 'lamwdesigner' +
              DirectorySeparator + 'Controls.native');
          end;
      ...
      with TJavaParser.Create(FFullJavaSrcPath + DirectorySeparator + 'Controls.java') do
        try         //"Controls.events" produced by FormWorkspace
          FPascalJNIInterfaceCode :=
            GetPascalJNIInterfaceCode(FPathToJavaTemplates +
            DirectorySeparator + 'Controls.events');
        finally
          Free;
        end;
      ...
      CreateDir(FAndroidProjectName + DirectorySeparator + 'jni' +
        DirectorySeparator + 'build-modes');
      CreateDir(FAndroidProjectName + DirectorySeparator + 'libs');
      CreateDir(FAndroidProjectName + DirectorySeparator + 'libs' +
        DirectorySeparator + 'armeabi');
      CreateDir(FAndroidProjectName + DirectorySeparator + 'libs' +
        DirectorySeparator + 'armeabi-v7a');
      CreateDir(FAndroidProjectName + DirectorySeparator + 'libs' +
        DirectorySeparator + 'x86');
      CreateDir(FAndroidProjectName + DirectorySeparator + 'libs' +
        DirectorySeparator + 'mips');
      CreateDir(FAndroidProjectName + DirectorySeparator + 'libs' +
        DirectorySeparator + 'arm64-v8a');
      CreateDir(FAndroidProjectName + DirectorySeparator + 'libs' +
        DirectorySeparator + 'x86_64');
      CreateDir(FAndroidProjectName + DirectorySeparator + 'obj');

      if FModuleType < 2 then
        CreateDir(FAndroidProjectName + DirectorySeparator + 'obj' +
          DirectorySeparator + 'controls');
      ...
      begin
        ...
        auxList.SaveToFile(FAndroidProjectName + DirectorySeparator +
          '.settings' + DirectorySeparator + 'org.eclipse.jdt.core.prefs');
        auxList.Clear;
      ...
      with TStringList.Create do
        try
          ...
          LoadFromFile(FPathToJavaTemplates + DirectorySeparator +
            'gdx' + DirectorySeparator + 'androidmanifest.txt');
          strAfterReplace := StringReplace(Text, 'dummyPackage',
            strPackName, [rfReplaceAll, rfIgnoreCase]);
...
begin
  Result := 'LAMW [NoGUI] Android Console Application' + LineEnding +
    '[Native Executable]' + LineEnding + 'using datamodule like form.' +
    LineEnding + 'The project is maintained by Lazarus.';
end;
...
begin
  try
    FModuleType := 2;
    //-1: gdx 0: GUI --- 1:NoGUI --- 2: NoGUI EXE Console  3: generic library
    FPathToClassName := '';
    ...
    begin
      ...
      begin
        ...
        list.Add(' ');
        if FPieChecked then
          list.Add('		NOTE 2: Project settings: Target Api = ' +
            FTargetApi + ' and PIE enabled!')
        else
          list.Add('		NOTE 2: Project settings: Targeg Api = ' +
            FTargetApi + ' and PIE  not enabled!');
        ...
        list.Add('exit ');
        list.Add(' ');
        list.Add('5. Copy your program file  "' + LowerCase(FSmallProjName) +
          '" from project folder "...\libs\armeabi\" to Emulator "/sdcard/tmp" ');
        list.Add(' ');
        list.Add('adb push C:\adt32\workspace\' + FSmallProjName +
          '\libs\armeabi\' + LowerCase(FSmallProjName) + '  /sdcard/tmp/' +
        ...
        list.Add(' ');
        list.Add('8. Now copy your programa file "' +
          LowerCase(FSmallProjName) + '" to an executable place ');
        list.Add(' ');
        list.Add('root@android:/sdcard/tmp # cp ' + LowerCase(FSmallProjName) +
          ' /data/local/tmp/' + LowerCase(FSmallProjName));
        list.Add(' ');
        list.Add('9. Go to folder /data/local/tmp and Change permission to run executable ');
        list.Add(' ');
        list.Add('root@android:/ # cd /data/local/tmp');
        list.Add('root@android:/data/local/tmp # chmod 755 ' +
          LowerCase(FSmallProjName));
        list.Add(' ');
        ...
        list.SaveToFile(FAndroidProjectName + DirectorySeparator +
        ...
        list.Add(' ');
        list.Add('2. Connect PC <---> Device via an USB cable  and  copy your program file  "'
          + LowerCase(FSmallProjName) +
          '" from project folder "...\libs\armeabi\" to Device folder "Download"');
        list.Add(' ');
        list.Add(
        ...
        list.Add(' ');
        list.Add('5. Copy [cat] your program file  "' + LowerCase(
          FSmallProjName) +
          '" from Device folder "Download" to internal "Terminal Emulador" storage folder');
        list.Add(' ');
        list.Add('$ cat /sdcard/Download/' + LowerCase(FSmallProjName) +
          ' > ' + LowerCase(FSmallProjName));
        list.Add(' ');
        list.Add('6. Change your program file  "' + LowerCase(FSmallProjName) +
          '" permission to "executable" mode');
        list.Add(' ');
...
begin
  try
    ...
    begin
        ...
        try
          ...
          begin
            ...
            if FileExists(FPathToJavaTemplates + DirectorySeparator +
              ...
              (not FileExists(FAndroidProjectName + DirectorySeparator +
              'res' + DirectorySeparator + 'xml' + DirectorySeparator +
              'support_provider_paths.xml')) then
            begin
              LoadFromFile(FPathToJavaTemplates + DirectorySeparator +
                'support' + DirectorySeparator + 'support_provider_paths.xml');
              SaveToFile(FAndroidProjectName + DirectorySeparator +
                'res' + DirectorySeparator + 'xml' + DirectorySeparator +
                'support_provider_paths.xml');
            end;
          end
          else
          begin
            if FileExists(FPathToJavaTemplates + DirectorySeparator +
              'jSupported.java') then
            begin
              LoadFromFile(FPathToJavaTemplates + DirectorySeparator +
                'jSupported.java');
              Strings[0] := 'package ' + strPackName + ';';  //replace dummy
          ...
          begin
            if FileExists(FPathToJavaTemplates + DirectorySeparator +
              'support' + DirectorySeparator + 'App.java') then
              LoadFromFile(FPathToJavaTemplates + DirectorySeparator +
                'support' + DirectorySeparator + 'App.java');
          end
          else
          begin
          ...
          begin
            if FileExists(FPathToJavaTemplates + DirectorySeparator +
              'support' + DirectorySeparator + 'jCommons.java') then
            begin
              LoadFromFile(FPathToJavaTemplates + DirectorySeparator +
      ...
      strAfterReplace := StringReplace(auxList.Text, 'dummyPackage',
        strPackName, [rfReplaceAll, rfIgnoreCase]);
      ...
      begin
        if FileExists(FPathToJavaTemplates + DirectorySeparator +
          'support' + DirectorySeparator + 'manifest_support_provider.txt') then
        begin
          providerList := TStringList.Create;
          providerList.LoadFromFile(FPathToJavaTemplates + DirectorySeparator +
            'support' + DirectorySeparator + 'manifest_support_provider.txt');
          supportProvider := StringReplace(providerList.Text,
            'dummyPackage', strPackName, [rfReplaceAll, rfIgnoreCase]);
...
var
  FIniFile: TIniFile;
begin
  FIniFile := TIniFile.Create(IncludeTrailingPathDelimiter(
    LazarusIDE.GetPrimaryConfigPath) + FIniFileName);
...
function TAndroidProjectDescriptor.TryNewJNIAndroidInterfaceCode(
  projectType: integer): boolean;
var
...
begin
  Result := 'LAMW [NoGUI] Android loadable module (.so)' + LineEnding +
    ...
    'No[!] Form Designer/Android and no Components Bridges!' +
    LineEnding + 'The project and library are maintained by Lazarus.';
end;
...
function TAndroidProjectDescriptor.HasBuildTools(platform: integer;
...
function TAndroidProjectDescriptor.TryGradleCompatibility(plugin: string;
...
function TAndroidProjectDescriptor.GetWorkSpaceFromForm(projectType: integer;
  out outTag: integer): boolean;
  ...
  begin
    ...
    begin
      ...
      i := 1;
      while sl.IndexOf(Result + IntToStr(i)) >= 0 do
        Inc(i);
...
begin
  ...
  try
    ...
    frm.ComboSelectProjectName.Text :=
      MakeUniqueName('AppLAMWProject', frm.ComboSelectProjectName.Items);
    ...
    begin
      ...
      frm.ComboSelectProjectName.Text :=
        MakeUniqueName('AppLAMWGdxProject', frm.ComboSelectProjectName.Items);
    ...
    begin
      ...
      frm.ComboSelectProjectName.Text :=
        MakeUniqueName('AppLAMWNoGUIProject', frm.ComboSelectProjectName.Items);
    ...
    begin
      ...
      frm.ComboSelectProjectName.Text :=
        MakeUniqueName('LamwConsoleApp', frm.ComboSelectProjectName.Items);
    ...
    begin
      ...
      //warning: full project name = path + name !
      ...
      begin
        instructionChip := ExtractFileDir(
      ...
      try
        ...
        begin
          if FModuleType < 2 then
            //-1:gdx 0: GUI project   1: NoGui project   2: NoGUI Exe
          begin
            ...
            strList.DelimitedText :=
              FPackagePrefaceName + '.' + LowerCase(FSmallProjName);
            for i := 0 to strList.Count - 1 do
            begin
              FFullJavaSrcPath :=
                FFullJavaSrcPath + DirectorySeparator + strList.Strings[i];
            ...
            if Pos('AppCompat', FAndroidTheme) > 0 then
            begin
              CopyFile(FPathToJavaTemplates + DirectorySeparator +
                'values' + DirectorySeparator + FAndroidTheme + '.xml',
                FAndroidProjectName + DirectorySeparator + 'res' +
                DirectorySeparator + 'values' + DirectorySeparator + 'styles.xml');
            end
            ...
            strList.Clear;
            strList.LoadFromFile(FPathToJavaTemplates + DirectorySeparator +
              'values-v14' + DirectorySeparator + 'styles.xml');

            if (intTargetApi >= 14) and (intTargetApi < 21) then
              strText := StringReplace(strList.Text, 'dummyTheme',
                'android:Theme.' + FAndroidTheme, [rfReplaceAll])
            else
              strText := StringReplace(strList.Text, 'dummyTheme',
                'android:Theme.DeviceDefault', [rfReplaceAll]);

            strList.Text := strText;
            strList.SaveToFile(FAndroidProjectName + DirectorySeparator +
              'res' + DirectorySeparator + 'values-v14' + DirectorySeparator +
              'styles.xml');

            intMinApi := StrToInt(FMinApi);

            CreateDir(FAndroidProjectName + DirectorySeparator +
              'res' + DirectorySeparator + 'values-v21');

            //replace "dummyTheme" ..res\values-v21
            strList.Clear;
            if (Pos('AppCompat', FAndroidTheme) <= 0) and
              (Pos('GDXGame', FAndroidTheme) <= 0) then  //not AppCompat
            begin
              CreateDir(FAndroidProjectName + DirectorySeparator +
                'res' + DirectorySeparator + 'values-v21');
              //replace "dummyTheme" ..res\values-v21
              ...
              begin
                strList.LoadFromFile(FPathToJavaTemplates + DirectorySeparator +
                  'values-v21' + DirectorySeparator + 'styles.xml');
              end
              else
                strList.LoadFromFile(FPathToJavaTemplates + DirectorySeparator +
                  'values-v21' + DirectorySeparator + 'styles-empty.xml');
              ...
              strList.Text := strText;
              strList.SaveToFile(FAndroidProjectName + DirectorySeparator +
                'res' + DirectorySeparator + 'values-v21' + DirectorySeparator +
                'styles.xml');
            end;

            CreateDir(FAndroidProjectName + DirectorySeparator +
              'res' + DirectorySeparator + 'layout');
            CopyFile(FPathToJavaTemplates + DirectorySeparator +
              'layout' + DirectorySeparator + 'activity_app.xml',
              FAndroidProjectName + DirectorySeparator + 'res' +
              DirectorySeparator + 'layout' + DirectorySeparator + 'activity_app.xml');
          ...
          begin
            ...
            begin
              strList.Clear;
              //dummy App.java - will be replaced with simonsayz's "App.java" template!
              strList.Add('package ' + FPackagePrefaceName + '.' +
                LowerCase(FSmallProjName) + ';');
          ...
          begin
            ...
            begin
              ...
              strList.Add(' ');
              strList.Add('//HINT: You can change/edit "App.java" and "' +
                FSmallProjName + '.java" ');
              ...
              strList.Add('   ' + FSmallProjName + ' m' + FSmallProjName +
                ';  //just for demo...');
              strList.Add('  ');
              ...
              strList.Add('       m' + FSmallProjName + ' = new ' +
                FSmallProjName + '(); //just for demo...');
              strList.Add('');
              strList.Add('       int sum = m' + FSmallProjName +
                '.getSum(2,3); //just for demo...');
              strList.Add('       Toast.makeText(getApplicationContext(), "m' +
                FSmallProjName + '.getSum(2,3) = "+ sum,Toast.LENGTH_LONG).show();');
              strList.Add(' ');
              strList.Add('       String mens = m' + FSmallProjName +
                '.getString(1); //just for demo...');
              strList.Add('       Toast.makeText(getApplicationContext(), "m' +
                FSmallProjName + '.getString(1) = "+ mens,Toast.LENGTH_LONG).show();');
              strList.Add(' ');
              strList.Add('   }');
              strList.Add('}');
              strList.SaveToFile(FFullJavaSrcPath + DirectorySeparator + 'App.java');
            ...
            if not FileExists(FFullJavaSrcPath + DirectorySeparator +
              FSmallProjName + '.java') then
            begin
              strList.Clear;
              strList.Add('package ' + FPackagePrefaceName + '.' +
                LowerCase(FSmallProjName) + ';');
              ...
              strList.Add('  static {');
              strList.Add('	  try {');
              strList.Add('	      System.loadLibrary("' + LowerCase(
                FSmallProjName) + '");');
              ...
              strList.Add('');
              strList.Add('}');
              strList.SaveToFile(FFullJavaSrcPath + DirectorySeparator +
                FSmallProjName + '.java');
            end;

            strList.Clear;

            if not FileExists(FAndroidProjectName + DirectorySeparator +
            ...
            begin
              ...
              strList.Add('            android:name="' + FPackagePrefaceName +
                '.' + LowerCase(FSmallProjName) + '.App"');
              strList.Add('            android:label="@string/app_name" >');
            ...
            begin
              ReadXMLFile(xmlAndroidManifest, FAndroidProjectName +
                DirectorySeparator + 'AndroidManifest.xml');
        ...
        begin
          strList.Clear;
          strList.Add('set Path=%PATH%;' + FPathToAntBin);
          //<--- thanks to andersonscinfo !  [set path=%path%;C:\and32\ant\bin]
          strList.Add('set JAVA_HOME=' + FPathToJavaJDK);
          ...
          strList.Add('pause');
          strList.SaveToFile(FAndroidProjectName + DirectorySeparator +
            'utils' + DirectorySeparator + 'list-target.bat');

          //need to pause on double-click use...
          strList.Clear;
          strList.Add('cmd /K list-target.bat');
          strList.SaveToFile(FAndroidProjectName + DirectorySeparator +
            'utils' + DirectorySeparator + 'paused-list-target.bat');
          ...
          strList.Add('pause');
          strList.SaveToFile(FAndroidProjectName + DirectorySeparator +
            'utils' + DirectorySeparator + 'create-avd-default.bat');

          //need to pause on double-click use...
          strList.Clear;
          strList.Add('cmd /k create-avd-default.bat');
          strList.SaveToFile(FAndroidProjectName + DirectorySeparator +
            'utils' + DirectorySeparator + 'paused-create-avd-default.bat');
          ...
          strList.Add('cd ' + FAndroidProjectName);
          strList.SaveToFile(FAndroidProjectName + DirectorySeparator +
            'launch-avd-default.bat');

          strList.Clear;
          strList.Add(FPathToAndroidSDK + 'platform-tools' +
            DirectorySeparator + 'adb uninstall ' + FPackagePrefaceName +
            '.' + LowerCase(FSmallProjName));
          strList.Add(FPathToAndroidSDK + 'platform-tools' +
            DirectorySeparator + 'adb install -r ' + FAndroidProjectName +
            DirectorySeparator + 'bin' + DirectorySeparator +
            FSmallProjName + '-debug.apk');
          strList.Add('pause');
          strList.SaveToFile(FAndroidProjectName + DirectorySeparator +
            'ant-adb-install-debug.bat');
          ...
          strList.Add(FPathToAndroidSDK + 'platform-tools' +
            DirectorySeparator + 'adb install -r ' + FAndroidProjectName +
            DirectorySeparator + 'build' + DirectorySeparator + 'outputs' +
            DirectorySeparator + 'apk' + DirectorySeparator + 'debug' +
            DirectorySeparator + FSmallProjName + '-' + instructionChip + '-debug.apk');
          strList.Add('pause');
          strList.SaveToFile(FAndroidProjectName + DirectorySeparator +
            'gradle-adb-install-debug.bat');
          ...
          strList.Add('pause');
          strList.SaveToFile(FAndroidProjectName + DirectorySeparator +
            'utils' + DirectorySeparator + 'logcat-error.bat');

          strList.Clear;
          strList.Add(FPathToAndroidSDK + 'platform-tools' +
            DirectorySeparator + 'adb logcat ActivityManager:I ' +
            FSmallProjName + '-' + FAntBuildMode + '.apk:D *:S');
          strList.Add('pause');
          strList.SaveToFile(FAndroidProjectName + DirectorySeparator +
            'utils' + DirectorySeparator + 'logcat-app-perform.bat');
          ...
          *)
          ...
          strList.Add('');
          strList.Add(
            '<!-- Tags required to enable conditional compilation in java sources -->');
          strList.Add('<property name="src.dir" location=".' +
            PathDelim + 'src' + PathDelim + AppendPathDelim(
            ReplaceChar(strPack, '.', PathDelim)) + '"/>');
          ...
          for i := cMinAPI to cMaxAPI do
          begin
            strList.Add('<condition property="ifdef_api' + IntToStr(i) +
              'up" value="/*">');
            strList.Add('  <equals arg1="${api' + IntToStr(i) + '}" arg2="false"/>');
            strList.Add('</condition>');
            strList.Add('<condition property="endif_api' + IntToStr(i) +
              'up" value="*/">');
            strList.Add('  <equals arg1="${api' + IntToStr(i) + '}" arg2="false"/>');
            strList.Add('</condition>');
          ...
          strList.Add('');
          strList.Add(
            '<!-- Copy & filter java sources for defined Android target, do not modify -->');
          ...
          strList.Add('   2.1 double click "install-' + FAntBuildMode +
            '.bat" to install the Apk on the Emulator [or real device]');
          strList.Add('   2.2 look for the App "' + FSmallProjName +
            '" in the Emulator [or real device] and click it!');
          strList.Add(' ');
          strList.Add('3. If AVD/Emulator is NOT running:');
          strList.Add('   3.1 If AVD/Emulator NOT exist:');
          strList.Add(
            '        3.1.1 double click "paused_create-avd-default.bat" to create the AVD ['
            + DirectorySeparator + 'utils folder]');
          strList.Add(
            '   3.2 double click "launch-avd-default.bat" to launch the Emulator [' +
            DirectorySeparator + 'utils  folder]');
          strList.Add('   3.3 look for the App "' + FSmallProjName +
            '" in the Emulator and click it!');
          strList.Add(' ');
          strList.Add('4. Log/Debug');
          strList.Add('   4.1 double click "logcat*.bat" to read logs and bugs! [' +
            DirectorySeparator + 'utils folder]');
          strList.Add(' ');
          ...
          strList.Add(' ');
          strList.Add('6. To find your Apk look for the "' + FSmallProjName +
            '-' + FAntBuildMode + '.apk" in ' + DirectorySeparator + 'bin folder!');
          strList.Add(' ');
          strList.Add(
            '7. Android Asset Packaging Tool: to know which files were packed in "' +
            FSmallProjName + '-' + FAntBuildMode + '.apk"');
          strList.Add('   7.1 double click "aapt.bat" [' + DirectorySeparator +
            'utils folder]');
          strList.Add(' ');
          strList.Add('8. To see all available Android targets in your system [' +
            DirectorySeparator + 'utils folder]');
          ...
          strList.Add(' ');
          strList.Add('12. Linux users: use "ant-build-debug.sh" , "install-' +
            FAntBuildMode +
            '.sh" , "uninstall.sh" and "logcat.sh" [thanks to Stephano!]');
          strList.Add(
          ...
          strList.SaveToFile(FAndroidProjectName + PathDelim + 'gradle.properties');
          ...
          strList.Add('MyDevelopmentUnitName');
          ...
          strList.Add('y');
          //Is <CN=FirstName LastName, OU=Development, O=MyExampleCompany, L=MyCity, ST=AK, C=WZ> correct?[no]:  y
          strList.Add('123456');
          //Enter key password for the Apk <aliasKey> <RETURN if same as keystore password>:
          strList.SaveToFile(FAndroidProjectName + DirectorySeparator +
            'keytool_input.txt');
          ...
          strList.Add('keytool -genkey -v -keystore ' + Lowercase(
            FSmallProjName) + '-release.keystore -alias ' + apk_aliaskey +
          ...
          strList.Add('cd ' + FAndroidProjectName);
          strList.Add('jarsigner -verify -verbose -certs ' +
            FAndroidProjectName + DirectorySeparator + 'bin' +
            DirectorySeparator + FSmallProjName + '-release.apk');
          strList.SaveToFile(FAndroidProjectName + DirectorySeparator +
            'ant-jarsigner-verify.bat');
          ...
          strList.Add('cd ' + FAndroidProjectName);
          strList.Add('jarsigner -verify -verbose -certs ' +
            FAndroidProjectName + DirectorySeparator + 'build' +
            DirectorySeparator + 'outputs' + DirectorySeparator +
            'apk' + DirectorySeparator + 'release' + DirectorySeparator +
            FSmallProjName + '-release.apk');
          strList.SaveToFile(FAndroidProjectName + DirectorySeparator +
            'gradle-jarsigner-verify.bat');
          ...
          strList.Add('');
          strList.Add(
            '3) Execute the [project] command "release-keystore.bat" or "release-keystore.sh" or "release-keystore-macos.sh" to get the "'
            + Lowercase(FSmallProjName) + '-release.keystore"');
          strList.Add('           warning: the file "' + Lowercase(
            FSmallProjName) +
            '-release.keystore" should be created only once [per application] otherwise it will fail [and NEVER delete it!]');
          strList.Add(' ');
          strList.Add(
          ...
          strList.Add('             hint: look for your generated "' +
            FSmallProjName +
            '-release.apk" in [project] folder "...\build\outputs\apk\release"');
          strList.Add(' ');
          strList.Add('');
          strList.Add(
            '6) [Ant]: Execute the [project] command "ant-build-release.bat" [.sh] to get the [release] signed Apk!"');
          strList.Add('          hint: look for your generated "' +
            FSmallProjName + '-release.apk" in [project] folder "...\bin"');
          strList.Add('');
          strList.Add('');
          strList.Add('Success! You can now upload your nice "' +
            FSmallProjName + '-release.apk" to "Google Play" [or others stores...]!');
          strList.Add('');
          ...
          strList.Add('....  by jmpessoa_hotmail_com');
          strList.SaveToFile(FAndroidProjectName + DirectorySeparator +
            'How_To_Get_Your_Signed_Release_Apk.txt');
          ...
          begin
            ...
            SaveShellScript(strList, FAndroidProjectName + PathDelim +
              'ant-build-debug.sh');
          end;
          ...
          strList.Clear;
          strList.Add(linuxPathToAdbBin + linuxDirSeparator +
            'adb uninstall ' + FPackagePrefaceName + '.' + LowerCase(FSmallProjName));
          ...
          strList.Add(linuxPathToAdbBin + linuxDirSeparator +
            'adb install -r ' + tempStr + linuxDirSeparator + 'bin' +
            linuxDirSeparator + FSmallProjName + '-debug.apk');
          SaveShellScript(strList, FAndroidProjectName + PathDelim +
            'ant-adb-install-debug.sh');
          ...
          {$endif}
          strList.Add(linuxPathToAdbBin + linuxDirSeparator +
            'adb install -r ' + tempStr + linuxDirSeparator +
            'build' + linuxDirSeparator + 'outputs' + linuxDirSeparator +
            'apk' + linuxDirSeparator + 'debug' + linuxDirSeparator +
            FSmallProjName + '-' + instructionChip + '-debug.apk');
          SaveShellScript(strList, FAndroidProjectName + PathDelim +
            'gradle-adb-install-debug.sh');
          ...
          strList.Add('cd ' + linuxAndroidProjectName);
          strList.Add('jarsigner -verify -verbose -certs ' +
            linuxAndroidProjectName + linuxDirSeparator + 'bin' +
            linuxDirSeparator + FSmallProjName + '-release.apk');
          SaveShellScript(strList, FAndroidProjectName + PathDelim +
            'ant-jarsigner-verify.sh');
          ...
          strList.Add('cd ' + linuxAndroidProjectName);
          strList.Add('jarsigner -verify -verbose -certs ' +
            linuxAndroidProjectName + linuxDirSeparator + 'build' +
            linuxDirSeparator + 'outputs' + linuxDirSeparator + 'apk' +
            linuxDirSeparator + 'release' + linuxDirSeparator +
            FSmallProjName + '-release.apk');
          SaveShellScript(strList, FAndroidProjectName + PathDelim +
            'gradle-jarsigner-verify.sh');
          ...
          strList.Add('jarsigner -verify -verbose -certs ' +
            ...
            FSmallProjName + '-release.apk');
          SaveShellScript(strList, FAndroidProjectName + PathDelim +
            'gradle-jarsigner-verify-macos.sh');
            ...
            begin
              ...
              begin
                if MessageDlg('Warning ', 'plugin "' + pluginVersion +
                  '", "build-tools "' + sdkBuildTools + '" require Gradle "' +
                  outgradleCompatible + '"' + sLineBreak + '[current: "' +
                  FGradleVersion + '"]', mtConfirmation, [mbOK, mbIgnore], 0) = mrOk then
                begin
                  gradleCompatible := outgradleCompatible;
                  ShowMessage('Please, update to Gradle "' +
                    outgradleCompatible + '" ' + sLineBreak +
              ...
              androidPluginNumber := GetVerAsNumber(pluginVersion);
              //ex. 3.0.0 --> 3000
              ...
              // strList.Add('        jcenter()'); deprecated.
              strList.Add(
              ...
              //end
              // end
              //else: each version of the Android Gradle Plugin now has a default version of the build tools. wrong.
              //target api version.
             ...
             { end
              ...
              strList.add('			   applicationId "' + FPackagePrefaceName +
                '.' + FSmallProjName + '"');
              ...
              begin
                ...
                strList.Add(
                  '    ' + directive + ' ''com.badlogicgames.gdx:gdx-box2d:1.9.10''');
                strList.Add(
                  '    ' + directive +
              ...
              begin
                ...
                strList.Add('    gradleVersion = ''' +
                  TryUndoFakeVersion(gradleCompatible) + '''');
                strList.Add('}');
              end
              else
              begin
              ...
              strList.Add('export PATH=' + linuxPathToAndroidSDK +
                'platform-tools' + ':$PATH');
              if FPathToGradle = '' then
              ...
              strList.Add(' ');
              strList.Add('    :: Where is my Apk? here: "' +
                FAndroidProjectName + '\build\outputs\apk"!');
              strList.Add(
              ...
              strList.Add('Congratulation!');
              strList.Add(' ');
              strList.Add('hint: where is my Apk? here: "' +
                FAndroidProjectName + '\build\outputs\apk"');
              strList.Add(' ');
              ...
              strList.Add('set PATH=%PATH%;%GRADLE_HOME%\bin');
              strList.Add('gradle wrapper');
              strList.SaveToFile(FAndroidProjectName + PathDelim +
                'gradle-making-wrapper.bat');
              ...
              sdkBuildTools := GetBuildTool(FMaxSdkPlatform);
              strList.Clear;
              strList.Add('set Path=%PATH%;' + FPathToAndroidSDK +
                'platform-tools;' + FPathToAndroidSDK + 'build-tools\' + sdkBuildTools);
              strList.Add('set GRADLE_HOME=' + FPathToGradle);
              strList.Add('set PATH=%PATH%;%GRADLE_HOME%\bin');
              ...
              strList.Add('zipalign -v -p 4 ' + FAndroidProjectName +
                '\build\outputs\apk\release\' + apkName +
                '-release-unsigned.apk ' + FAndroidProjectName +
                '\build\outputs\apk\release\' + apkName +
                '-release-unsigned-aligned.apk');
              strList.Add('apksigner sign --ks ' + FAndroidProjectName +
                '\' + Lowercase(FSmallProjName) +
                '-release.keystore --ks-pass pass:123456 --key-pass pass:123456 --out ' +
                FAndroidProjectName + '\build\outputs\apk\release\' +
                FSmallProjName + '-release.apk ' + FAndroidProjectName +
                '\build\outputs\apk\release\' + apkName +
                '-release-unsigned-aligned.apk');
              strList.SaveToFile(FAndroidProjectName + PathDelim +
              ...
              strList.Add('set PATH=%PATH%;%GRADLE_HOME%\bin');
              strList.Add('zipalign -v -p 4 ' + FAndroidProjectName +
                '\build\outputs\apk\release\' + FSmallProjName +
                '-universal-release-unsigned.apk ' + FAndroidProjectName +
                '\build\outputs\apk\release\' + FSmallProjName +
              ...
              strList.Add('apksigner sign --ks ' + FAndroidProjectName +
                ...
                '-universal-release-unsigned-aligned.apk');
              strList.SaveToFile(FAndroidProjectName + PathDelim +
                'gradle-local-universal-apksigner.bat');
              ...
              strList.Add('gradle clean bundle --info');
              SaveShellScript(strList, FAndroidProjectName + PathDelim +
                'gradle-local-build-bundle.sh');

              strList.Clear;
              strList.Add('export PATH=' + linuxPathToAndroidSDK +
                'platform-tools' + ':$PATH');
              strList.Add('export PATH=' + linuxPathToAndroidSDK +
                'build-tools/' + sdkBuildTools + ':$PATH');
              strList.Add('export GRADLE_HOME=' + linuxPathToGradle);
              strList.Add('export PATH=$PATH:$GRADLE_HOME/bin');
              strList.Add('zipalign -v -p 4 ' + linuxAndroidProjectName +
                '/build/outputs/apk/release/' + apkName +
                '-release-unsigned.apk ' + linuxAndroidProjectName +
                '/build/outputs/apk/release/' + apkName +
                '-release-unsigned-aligned.apk');
              strList.Add('apksigner sign --ks ' + linuxAndroidProjectName +
                '/' + Lowercase(FSmallProjName) +
                '-release.keystore --ks-pass pass:123456 --key-pass pass:123456 --out ' +
                linuxAndroidProjectName + '/build/outputs/apk/release/' +
                FSmallProjName + '-release.apk ' + linuxAndroidProjectName +
                '/build/outputs/apk/release/' + apkName +
                '-release-unsigned-aligned.apk');
              SaveShellScript(strList, FAndroidProjectName + PathDelim +
                'gradle-local-apksigner.sh');
              ...
              strList.Add('zipalign -v -p 4 ' + linuxAndroidProjectName +
                '/build/outputs/apk/release/' + FSmallProjName +
                '-universal-release-unsigned.apk ' + linuxAndroidProjectName +
                '/build/outputs/apk/release/' + FSmallProjName +
                '-universal-release-unsigned-aligned.apk');
              strList.Add('apksigner sign --ks ' + linuxAndroidProjectName +
                '/' + Lowercase(FSmallProjName) +
                ...
                '-universal-release-unsigned-aligned.apk');
              SaveShellScript(strList, FAndroidProjectName + PathDelim +
                'gradle-local-universal-apksigner.sh');
          ...
          begin
            ShowMessage('Fail! Sorry... You need install SDK "build-tools" '
              + IntToStr(Self.FMaxSdkPlatform) + '.x.y');
          end;
        end;
        Result := True;
      except
...
begin
  ...
  if FPathToClassName = '' then
    FPathToClassName := StringReplace(FPackagePrefaceName, '.', '/',
      [rfReplaceAll]) + '/' + LowerCase(FSmallProjName) + '/' + FJavaClassName;
  //ex. 'com/example/appasynctaskdemo1/Controls'
  ...
  if FModuleType < 2 then    {-1:gdx 0: GUI; 1: NoGUI; 2: NoGUI EXE Console}
    AProject.CustomData.Values['Package'] :=
      FPackagePrefaceName + '.' + LowerCase(FSmallProjName);
  ...
  sourceList.Add('{hint: Pascal files location: ...' + DirectorySeparator +
    FSmallProjName + DirectorySeparator + 'jni }');

  if FModuleType = 2 then  //console executavel
    sourceList.Add('program ' + LowerCase(FSmallProjName) + '; ' +
      ' //[by LAMW: Lazarus Android Module Wizard: ' + DateTimeToStr(Now) + ']')
  else if FModuleType = 3 then
    sourceList.Add('library ' + LowerCase(FSmallProjName) + '; ' +
      ' //[by LAMW: Lazarus Android Module Wizard: ' + DateTimeToStr(Now) + ']')
  else
  ...
  begin
    ...
    sourceList.Add(' ');
    sourceList.Add(
      'procedure TAndroidConsoleApp.CreateForm(InstanceClass: TComponentClass; out Reference);');
    sourceList.Add('var');
    sourceList.Add('  Instance: TComponent;');
    sourceList.Add('begin');
    sourceList.Add('  Instance := TComponent(InstanceClass.NewInstance);');
  ...
  begin
    ...
    //com.example.appasynctaskdemo1
    sourceList.Add('  gjClassName:= ''' + FPathToClassName + ''';');
    //com/example/appasynctaskdemo1/Controls
  ...
  begin  //generic library     // FModuleType = 3
    ...
    headerList.Add(' ');
    headerList.Add('  function Sum(a: longint; b: longint): longint; cdecl; external ''lib'
      + LowerCase(FSmallProjName) + '.so'' name ''Sum'';');
    headerList.Add(' ');
    headerList.Add('implementation');
    headerList.Add(' ');
    headerList.Add('end.');
    headerList.SaveToFile(projDir + 'libs' + DirectorySeparator +
      LowerCase(FSmallProjName) + '_h.pas');
    headerList.Free;
  ...
  AProject.Flags := AProject.Flags - [pfMainUnitHasCreateFormStatements,
    pfMainUnitHasTitleStatement, pfLRSFilesInOutputDirectory];
  ...
  begin
    ...
    PathToNdkPlatformsArm := FPathToAndroidNDK + 'platforms' +
      DirectorySeparator + androidPlatformApi + DirectorySeparator +
      'arch-arm' + DirectorySeparator + 'usr' + DirectorySeparator + 'lib';

    PathToNdkPlatformsAarch64 :=
      FPathToAndroidNDK + 'platforms' + DirectorySeparator +
      androidPlatformApi + DirectorySeparator + 'arch-arm64' +
      DirectorySeparator + 'usr' + DirectorySeparator + 'lib';

    PathToNdkPlatformsX86 := FPathToAndroidNDK + 'platforms' +
      DirectorySeparator + androidPlatformApi + DirectorySeparator +
      'arch-x86' + DirectorySeparator + 'usr' + DirectorySeparator + 'lib';

    PathToNdkPlatformsX86_64 :=
      FPathToAndroidNDK + 'platforms' + DirectorySeparator +
      androidPlatformApi + DirectorySeparator + 'arch-x86_64' +
      DirectorySeparator + 'usr' + DirectorySeparator + 'lib';

    PathToNdkPlatformsMips := FPathToAndroidNDK + 'platforms' +
      DirectorySeparator + androidPlatformApi + DirectorySeparator +
      'arch-mips' + DirectorySeparator + 'usr' + DirectorySeparator + 'lib';
  end
  else //NDK >= 22
  begin
    //C:\android\android-ndk-r22b\toolchains\llvm\prebuilt\windows-x86_64\sysroot\usr\lib\arm-linux-androideabi\22
    PathToNdkPlatformsArm := ConcatPaths(
      [FPathToAndroidNDK, 'toolchains', 'llvm', 'prebuilt', FPrebuildOSys,
      'sysroot', 'usr', 'lib', 'arm-linux-androideabi', FNdkApi]);
  ...
  begin
    pathToNdkToolchainsArm :=
      FPathToAndroidNDK + 'toolchains' + DirectorySeparator +
      'arm-linux-androideabi-4.4.3' + DirectorySeparator + 'prebuilt' +
      DirectorySeparator + osys + DirectorySeparator + 'lib' +
      DirectorySeparator + 'gcc' + DirectorySeparator + 'arm-linux-androideabi' +
      DirectorySeparator + '4.4.3';

    pathToNdkToolchainsBinArm :=
      FPathToAndroidNDK + 'toolchains' + DirectorySeparator +
      'arm-linux-androideabi-4.4.3' + DirectorySeparator + 'prebuilt' +
      DirectorySeparator + osys + DirectorySeparator + 'bin';

    pathToNdkToolchainsX86 :=
      FPathToAndroidNDK + 'toolchains' + DirectorySeparator +
      'x86-4.4.3' + DirectorySeparator + 'prebuilt' + DirectorySeparator +
      osys + DirectorySeparator + 'lib' + DirectorySeparator + 'gcc' +
      DirectorySeparator + 'i686-android-linux' + DirectorySeparator + '4.4.3';

    pathToNdkToolchainsBinX86 :=
      FPathToAndroidNDK + 'toolchains' + DirectorySeparator +
      'x86-4.4.3' + DirectorySeparator + 'prebuilt' + DirectorySeparator +
      osys + DirectorySeparator + 'bin';

  end
  else if {(FNDK = '9') or (FNDK = '10') or (FNDK = '10c')} (FNDKIndex > 0) and
    (FNDKIndex < 3) then          //arm-linux-androideabi-4.9
  begin
    pathToNdkToolchainsArm :=
      FPathToAndroidNDK + 'toolchains' + DirectorySeparator +
      'arm-linux-androideabi-4.6' + DirectorySeparator + 'prebuilt' +
      DirectorySeparator + osys + DirectorySeparator + 'lib' +
      DirectorySeparator + 'gcc' + DirectorySeparator + 'arm-linux-androideabi' +
      DirectorySeparator + '4.6';
    pathToNdkToolchainsBinArm :=
      FPathToAndroidNDK + 'toolchains' + DirectorySeparator +
      'arm-linux-androideabi-4.6' + DirectorySeparator + 'prebuilt' +
      DirectorySeparator + osys + DirectorySeparator + 'bin';
    ...
    pathToNdkToolchainsX86 :=
      ...
      osys + DirectorySeparator + 'lib' + DirectorySeparator + 'gcc' +
      DirectorySeparator + 'i686-android-linux' + DirectorySeparator + '4.6';
  ...
  else if {FNDK = '10e'} {FNDK = '11c'} (FNDKIndex >= 3) and (FNDKIndex < 5) then
    //arm-linux-androideabi-4.9
  begin
    pathToNdkToolchainsArm :=
      FPathToAndroidNDK + 'toolchains' + DirectorySeparator +
      'arm-linux-androideabi-4.9' + DirectorySeparator + 'prebuilt' +
      DirectorySeparator + osys + DirectorySeparator + 'lib' +
      DirectorySeparator + 'gcc' + DirectorySeparator + 'arm-linux-androideabi' +
      DirectorySeparator + '4.9';


    pathToNdkToolchainsAarch64 :=
      FPathToAndroidNDK + 'toolchains' + DirectorySeparator +
      'aarch64-linux-android-4.9' + DirectorySeparator + 'prebuilt' +
      DirectorySeparator + osys + DirectorySeparator + 'lib' +
      DirectorySeparator + 'gcc' + DirectorySeparator + 'aarch64-linux-android' +
      DirectorySeparator + '4.9';
    ...
    pathToNdkToolchainsX86 :=
      ...
      osys + DirectorySeparator + 'lib' + DirectorySeparator + 'gcc' +
      DirectorySeparator + 'i686-android-linux' + DirectorySeparator + '4.9';

    pathToNdkToolchainsX86_64 :=
      FPathToAndroidNDK + 'toolchains' + DirectorySeparator +
      'x86_64-4.9' + DirectorySeparator + 'prebuilt' + DirectorySeparator +
      osys + DirectorySeparator + 'lib' + DirectorySeparator + 'gcc' +
      DirectorySeparator + 'x86_64-android-linux' + DirectorySeparator + '4.9';

    pathToNdkToolchainsMips :=
      FPathToAndroidNDK + 'toolchains' + DirectorySeparator +
      'mipsel-linux-android-4.9' + DirectorySeparator + 'prebuilt' +
      DirectorySeparator + osys + DirectorySeparator + 'lib' +
      DirectorySeparator + 'gcc' + DirectorySeparator + 'mipsel-linux-android' +
      DirectorySeparator + '4.9';
    ...
    pathToNdkToolchainsBinX86_64 :=
      FPathToAndroidNDK + 'toolchains' + DirectorySeparator +
      'x86_64-4.9' + DirectorySeparator + 'prebuilt' + DirectorySeparator +
      osys + DirectorySeparator + 'bin';

    pathToNdkToolchainsBinMips :=
      FPathToAndroidNDK + 'toolchains' + DirectorySeparator +
      'mipsel-linux-android-4.9' + DirectorySeparator + 'prebuilt' +
      DirectorySeparator + osys + DirectorySeparator + 'bin';

  end
  else if {FNDK = '>11'} FNDKIndex >= 5 then          //arm-linux-androideabi-4.9
  begin
    pathToNdkToolchainsArm :=
      FPathToAndroidNDK + 'toolchains' + DirectorySeparator +
      'arm-linux-androideabi-4.9' + DirectorySeparator + 'prebuilt' +
    ...
    pathToNdkToolchainsX86 :=
      ...
      'x86-4.9' + DirectorySeparator + 'prebuilt' + DirectorySeparator +
      osys + DirectorySeparator + 'lib' + DirectorySeparator + 'gcc' +
      DirectorySeparator + 'i686-android-linux' + DirectorySeparator + '4.9.x';

    pathToNdkToolchainsX86_64 :=
      FPathToAndroidNDK + 'toolchains' + DirectorySeparator +
      'x86_64-4.9' + DirectorySeparator + 'prebuilt' + DirectorySeparator +
      osys + DirectorySeparator + 'lib' + DirectorySeparator + 'gcc' +
      DirectorySeparator + 'x86_64-android-linux' + DirectorySeparator + '4.9.x';
  ...
  if PathToNdkPlatformsArm <> '' then
    ...
    libraries_arm := pathToNdkToolchainsArm;

  if PathToNdkPlatformsAarch64 <> '' then
    libraries_aarch64 := PathToNdkPlatformsAarch64 + ';' + pathToNdkToolchainsAarch64
  else
    libraries_aarch64 := pathToNdkToolchainsAarch64;

  if PathToNdkPlatformsX86 <> '' then
    libraries_x86 := PathToNdkPlatformsX86 + ';' + pathToNdkToolchainsX86
  else
    libraries_x86 := pathToNdkToolchainsX86;

  if PathToNdkPlatformsX86_64 <> '' then
    libraries_x86_64 := PathToNdkPlatformsX86_64 + ';' + pathToNdkToolchainsX86_64
  else
    libraries_x86_64 := pathToNdkToolchainsX86_64;

  if PathToNdkPlatformsMips <> '' then
    libraries_mips := PathToNdkPlatformsMips + ';' + pathToNdkToolchainsMips
  else
    libraries_mips := pathToNdkToolchainsMips;


  //https://developer.android.com/ndk/guides/abis
  auxStr := 'armeabi'; //ARMv6
  auxInstr := LowerCase(FInstructionSet);
  if auxInstr = 'armv7a' then
    auxStr := 'armeabi-v7a';
  if auxInstr = 'x86' then
    auxStr := 'x86';
  if auxInstr = 'x86_64' then
    auxStr := 'x86_64';
  if auxInstr = 'mipsel' then
    auxStr := 'mips';
  if auxInstr = 'armv8' then
    auxStr := 'arm64-v8a';

  if Self.FAndroidTheme = 'GDXGame' then
  begin
    CopyFile(FPathToJavaTemplates + DirectorySeparator + 'gdx' +
      DirectorySeparator + auxStr + DirectorySeparator + 'libgdx.so',
      FPathToJNIFolder + DirectorySeparator + 'libs' + DirectorySeparator +
      auxStr + DirectorySeparator + 'libgdx.so');

    CopyFile(FPathToJavaTemplates + DirectorySeparator + 'gdx' +
      DirectorySeparator + auxStr + DirectorySeparator + 'libgdx-box2d.so',
      FPathToJNIFolder + DirectorySeparator + 'libs' + DirectorySeparator +
      auxStr + DirectorySeparator + 'libgdx-box2d.so');
  end;
  ...
  customOptions_armV7a_VFPv3 :=
    customOptions_armV7a_VFPv3 + ' -XParm-linux-androideabi-';
  ...
  if Pos('armeabi', auxStr) > 0 then
    customOptions_default := customOptions_default + ' -XParm-linux-androideabi-' +
      ' -FD' + pathToNdkToolchainsBinArm
  else if Pos('arm64', auxStr) > 0 then
    customOptions_default :=
      customOptions_default + ' -XPaarch64-linux-android-' + ' -FD' +
      pathToNdkToolchainsBinAarch64
  else if Pos('x86_64', auxStr) > 0 then
    customOptions_default :=
      customOptions_default + ' -XPx86_64-linux-android-' + ' -FD' +
      pathToNdkToolchainsBinX86_64
  else if Pos('x86', auxStr) > 0 then
    customOptions_default := customOptions_default + ' -XPi686-linux-android-' +
      ' -FD' + pathToNdkToolchainsBinX86
  else if Pos('mips', auxStr) > 0 then
    customOptions_default := customOptions_default + ' -XPmipsel-linux-android-' +
      ' -FD' + pathToNdkToolchainsBinMips;
  ...
  if FModuleType < 2 then
    auxList.SaveToFile(FPathToJNIFolder + DirectorySeparator + 'jni' +
      DirectorySeparator + 'build-modes' + DirectorySeparator + 'build_x86.txt')
  else
    auxList.SaveToFile(FPathToJNIFolder + DirectorySeparator +
      'build-modes' + DirectorySeparator + 'build_x86.txt');
  ...
  if FModuleType < 2 then
    auxList.SaveToFile(FPathToJNIFolder + DirectorySeparator + 'jni' +
      DirectorySeparator + 'build-modes' + DirectorySeparator + 'build_x86_64.txt')
  else
    auxList.SaveToFile(FPathToJNIFolder + DirectorySeparator +
      'build-modes' + DirectorySeparator + 'build_x86_64.txt');
  ...
  if FModuleType < 2 then
    auxList.SaveToFile(FPathToJNIFolder + DirectorySeparator + 'jni' +
      DirectorySeparator + 'build-modes' + DirectorySeparator + 'build_mipsel.txt')
  else
    auxList.SaveToFile(FPathToJNIFolder + DirectorySeparator +
      'build-modes' + DirectorySeparator + 'build_mipsel.txt');
  ...
  if FModuleType < 2 then
    auxList.SaveToFile(FPathToJNIFolder + DirectorySeparator + 'jni' +
      DirectorySeparator + 'build-modes' + DirectorySeparator + 'build_armV6.txt')
  else
    auxList.SaveToFile(FPathToJNIFolder + DirectorySeparator +
      'build-modes' + DirectorySeparator + 'build_armV6.txt');
  ...
  if FModuleType < 2 then
    auxList.SaveToFile(FPathToJNIFolder + DirectorySeparator + 'jni' +
      DirectorySeparator + 'build-modes' + DirectorySeparator + 'build_armV7a.txt')
  else
    auxList.SaveToFile(FPathToJNIFolder + DirectorySeparator +
      'build-modes' + DirectorySeparator + 'build_armV7a.txt');
  ...
  if FModuleType < 2 then
    auxList.SaveToFile(FPathToJNIFolder + DirectorySeparator + 'jni' +
      DirectorySeparator + 'build-modes' + DirectorySeparator + 'build_armV7a_VFPv3.txt')
  else
    auxList.SaveToFile(FPathToJNIFolder + DirectorySeparator +
      'build-modes' + DirectorySeparator + 'build_armV7a_VFPv3.txt');
  ...
  if FModuleType < 2 then
    auxList.SaveToFile(FPathToJNIFolder + DirectorySeparator + 'jni' +
      DirectorySeparator + 'build-modes' + DirectorySeparator + 'build_arm64.txt')
  else
    auxList.SaveToFile(FPathToJNIFolder + DirectorySeparator +
      'build-modes' + DirectorySeparator + 'build_arm64.txt');
  ...
  if FModuleType < 2 then
    auxList.SaveToFile(FPathToJNIFolder + DirectorySeparator + 'jni' +
      DirectorySeparator + 'build-modes' + DirectorySeparator + 'readme.txt')
  else
    auxList.SaveToFile(FPathToJNIFolder + DirectorySeparator +
      'build-modes' + DirectorySeparator + 'readme.txt');

  if FModuleType < 2 then
  begin
    AProject.LazCompilerOptions.TargetFilename :=
      '..' + DirectorySeparator + 'libs' + DirectorySeparator + auxStr +
      DirectorySeparator + 'lib' + LowerCase(FJavaClassName){+'.so'};

    AProject.LazCompilerOptions.UnitOutputDirectory :=
      '..' + DirectorySeparator + 'obj' + DirectorySeparator + LowerCase(FJavaClassName);
    {-FU}
  ...
  begin
    AProject.LazCompilerOptions.TargetFilename :=
      'libs' + DirectorySeparator + auxStr + DirectorySeparator +
      LowerCase(FSmallProjName);
...
function TAndroidProjectDescriptor.IsTemplateProject(tryTheme: string;
  out outAndroidTheme: string): boolean;
var
  p: integer;
begin
  if DirectoryExists(GetPathToSmartDesigner() + PathDelim + 'AppTemplates' +
    PathDelim + tryTheme) then
  begin
...
begin
  ...
  begin
    ...
    pathToTemplate := GetPathToSmartDesigner() + PathDelim +
      'AppTemplates' + PathDelim + FAndroidTemplateTheme;
    ...
    try
      ...
      begin
        ...
        CopyFile(templateFiles.Strings[i], FPathToJNIFolder + PathDelim +
          'assets' + PathDelim + fileName);
      end;
    finally
      templateFiles.Free;
    end;
    ...
    templateFiles := TStringList.Create;
    try
      FindAllFiles(templateFiles, pathToTemplate + PathDelim + 'res' +
        PathDelim + 'drawable', '*.*', False);
      Count := templateFiles.Count;
      for i := 0 to Count - 1 do
      begin
        fileName := ExtractFileName(templateFiles.Strings[i]);
        if Pos('ic_launcher', fileName) <= 0 then
          CopyFile(templateFiles.Strings[i], FPathToJNIFolder +
            PathDelim + 'res' + PathDelim + 'drawable' + PathDelim + fileName);
      end;
    finally
      templateFiles.Free;
    end;
    ...
    try
      ...
      begin
        ...
        if Pos('ic_launcher', fileName) <= 0 then
          CopyFile(templateFiles.Strings[i], FPathToJNIFolder + PathDelim +
            'res' + PathDelim + 'drawable-hdpi' + PathDelim + fileName);
      end;
    finally
      templateFiles.Free;
    end;
    ...
    try
      ...
      begin
        ...
        if Pos('ic_launcher', fileName) <= 0 then
          CopyFile(templateFiles.Strings[i], FPathToJNIFolder + PathDelim +
            'res' + PathDelim + 'drawable-mdpi' + PathDelim + fileName);
      end;
    finally
      templateFiles.Free;
    end;
    ...
    try
      FindAllFiles(templateFiles, pathToTemplate + PathDelim + 'res' +
        PathDelim + 'drawable-xhdpi', '*.*', False);
      Count := templateFiles.Count;
      for i := 0 to Count - 1 do
      begin
        fileName := ExtractFileName(templateFiles.Strings[i]);
        if Pos('ic_launcher', fileName) <= 0 then
          CopyFile(templateFiles.Strings[i], FPathToJNIFolder + PathDelim +
            'res' + PathDelim + 'drawable-xhdpi' + PathDelim + fileName);
      end;
    finally
      templateFiles.Free;
    end;
    ...
    try
      FindAllFiles(templateFiles, pathToTemplate + PathDelim + 'res' +
        PathDelim + 'drawable-xxhdpi', '*.*', False);
      Count := templateFiles.Count;
      for i := 0 to Count - 1 do
      begin
        fileName := ExtractFileName(templateFiles.Strings[i]);
        if Pos('ic_launcher', fileName) <= 0 then
          CopyFile(templateFiles.Strings[i], FPathToJNIFolder + PathDelim +
            'res' + PathDelim + 'drawable-xxhdpi' + PathDelim + fileName);
      end;
    finally
      templateFiles.Free;
    end;
    ...
    try
      ...
      begin
        ...
        if Pos('ic_launcher', fileName) <= 0 then
          CopyFile(templateFiles.Strings[i], FPathToJNIFolder + PathDelim +
            'res' + PathDelim + 'drawable-ldpi' + PathDelim + fileName);
      end;
    finally
      templateFiles.Free;
    end;
    ...
    try
      ...
      begin
        fileName := ExtractFileName(templateFiles.Strings[i]);
        CopyFile(templateFiles.Strings[i], FPathToJNIFolder + PathDelim +
          'res' + PathDelim + 'raw' + PathDelim + fileName);
      end;
    finally
      templateFiles.Free;
    end;
    ...
    try
      ...
      begin
        fileName := ExtractFileName(templateFiles.Strings[i]);
        CopyFile(templateFiles.Strings[i], FPathToJNIFolder + PathDelim +
          'res' + PathDelim + 'xml' + PathDelim + fileName);
      end;
    finally
      templateFiles.Free;
    end;
    ...
    templateFiles := TStringList.Create;
    try
      FindAllFiles(templateFiles, pathToTemplate + PathDelim + 'jni',
        '*.pas;*.lfm', False);
      Count := templateFiles.Count;
      for i := 0 to Count - 1 do
      begin
        fileName := ExtractFileName(templateFiles.Strings[i]);
        CopyFile(templateFiles.Strings[i], FPathToJNIFolder +
          PathDelim + 'jni' + PathDelim + fileName);
        if Pos('.pas', fileName) > 0 then
        begin
          unitFile := LazarusIDE.ActiveProject.CreateProjectFile(
            FPathToJNIFolder + PathDelim + 'jni' + PathDelim + fileName);
...
function TAndroidFileDescPascalUnitWithResource.CreateSource(
...
begin
  ...
  if ModuleType < 2 then
    ...
    sourceList.Add('{hint: Pascal files location: ...' + DirectorySeparator +
      SmallProjName + DirectorySeparator + 'jni }')
  else
    //sourceList.Add('{Hint: save all files to location: ' +PathToJNIFolder +'}');
    sourceList.Add('{hint: Pascal files location: ...' + DirectorySeparator +
      SmallProjName + DirectorySeparator + '}');
...
function TAndroidFileDescPascalUnitWithResourceGDX.CreateSource(
  const Filename: string; const SourceName: string; const ResourceName: string): string;
var
  sourceList: TStringList;
  uName: string;
...
function TAndroidFileDescPascalUnitWithResource.GetInterfaceSource(
  const Filename: string; const SourceName: string; const ResourceName: string): string;
var
  strList: TStringList;
begin
...
function TAndroidFileDescPascalUnitWithResourceGDX.GetInterfaceSource(
  const Filename: string; const SourceName: string; const ResourceName: string): string;
var
  strList: TStringList;
begin
