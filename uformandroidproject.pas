...
uses
  ...
  EditBtn, ShellCtrls, Menus, Clipbrd, types,Process, uRegisterForm;
  ...
  begin
    ...
    begin
       ...
       begin
          ...
          paramType:= '';
          paramName:= GetParamNameHack(funcParam, paramType);
...
begin
  ...
  listPascal.Add(' ');
  listPascal.Add('uses');
  listPascal.Add('  Classes, SysUtils, And_jni, And_jni_Bridge, Laz_And_Controls;'); //{,LResources}
  listPascal.Add(' ');
  listPascal.Add('type');
  listPascal.Add(' ');
  ...
  *)
  ...
  if Pos('_', FHackCreateParam) > 0 then
    listPascal.Add('  FjObject:= jCreate('+StringReplace(FHackCreateParam,'_','F',[rfReplaceAll])+');')   //** add F
  else
  begin
    listProperties.StrictDelimiter:= True;
    listProperties.Delimiter:= ',';
    ...
    listPascal.Add('  FjObject:= jCreate('+listProperties.DelimitedText+');');   //** add F
  end;
  ...
  if FProjectModel = 'jVisualControl' then
  begin
   listPascal.Add('  if FParentPanel <> nil then');
   listPascal.Add('  begin');
  ...
  begin
   ...
   listPascal.Add('procedure '+FJavaClassName+'.SetParamWidth(Value: TLayoutParams);');
   listPascal.Add('var');
   listPascal.Add('  side: TSide;');
   listPascal.Add('begin');
   listPascal.Add('  if jForm(Owner).Orientation = jForm(Owner).App.Orientation then');
   listPascal.Add('    side:= sdW');
   listPascal.Add('  else');
   listPascal.Add('    side:= sdH;');
   ...
   listPascal.Add('end;');
   listPascal.Add('   ');
   listPascal.Add('procedure '+FJavaClassName+'.SetParamHeight(Value: TLayoutParams);');
   listPascal.Add('var');
   listPascal.Add('  side: TSide;');
   listPascal.Add('begin');
   listPascal.Add('  if jForm(Owner).Orientation = gApp.Orientation then');
   listPascal.Add('    side:= sdH');
   listPascal.Add('  else');
   listPascal.Add('    side:= sdW;');
   ...
   listPascal.Add('  SetParamWidth(FLParamWidth);');
   listPascal.Add('  SetParamHeight(FLParamHeight);');
   listPascal.Add('  '+FJavaClassName+'_SetLayoutAll(jForm(Owner).App.Jni.jEnv, jForm(Owner).App.Jni.jThis, FjObject, Self.AnchorId);');
   listPascal.Add('end;');
   listPascal.Add('   ');
...
var
  ...
  txtCaption, projectName: string;
begin
   txtCaption:= txt;
   if Pos('jControl', txtCaption) > 0 then
   begin
     projectName:= 'MyControl';
     strList:= TStringList.Create;
     ...
     strList.Add(' ');
     strList.Add('class j'+projectName+' /*extends ...*/ {');
     strList.Add('  ');
     ...
     strList.Add('  ');
     strList.Add('    public j'+projectName+'(Controls _ctrls, long _Self) { //Add more others news "_xxx" params if needed!');
   ...
   begin
     ...
     strList.Add('  ');
     strList.Add('class j'+projectName+' extends TextView /*dummy*/ { //please, fix what GUI object will be extended!');
     strList.Add('   ');
     strList.Add('   private long       pascalObj = 0;    // Pascal Object');
     ...
     strList.Add('  //GUIDELINE: please, preferentially, init all yours params names with "_", ex: int _flag, String _hello ...');
     strList.Add('  ');
     strList.Add('   public j'+projectName+'(Controls _ctrls, long _Self) { //Add more others news "_xxx"p arams if needed!');
     strList.Add('      super(_ctrls.activity);');
     strList.Add('      context   = _ctrls.activity;');
     strList.Add('      pascalObj = _Self;');
     strList.Add('      controls  = _ctrls;');
