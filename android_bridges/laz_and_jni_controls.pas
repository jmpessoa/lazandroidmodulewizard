...
begin
  ...
  jParamsContext[0].l:= gApp.GetContext(); //get Activity/Context jObject

  FjObject := Create_jObjectLocalRefA(FjClass, 'Landroid/content/Context;', jParamsContext);

  jParamsInt[0].i:= GetLayoutParams(gApp.GetContext(), FLParamWidth, sdW);  //W
  jParamsInt[1].i:= GetLayoutParams(gApp.GetContext(), FLParamHeight, sdH);  //H
...
begin
  ...
  if FInitialized then
  begin
    //get method :: class ref, method name, (param signature)return type
