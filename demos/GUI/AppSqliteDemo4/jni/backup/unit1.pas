...
type
  ...
  TAndroidModule1 = class(jForm)
      ...
      procedure jTextView2Click(Sender: TObject);
    private
...
begin
   ...
   jSqliteDataAccess1.Select('SELECT * FROM '+  FTableName, false);
   ...
   begin
     ...
     if sqlCursor.GetValueAsInteger('_ID') = 5 then
      showmessage('Found _ID = 5')
     else
      showmessage(sqlCursor.GetValueToString('_ID') + '-' +
                  sqlCursor.GetValueToString('TITLE') + '-' +
                  sqlCursor.GetValueToString('AUTOR') + '-' +
                  sqlCursor.GetValueToString('COUNT') + '-' +
                  sqlCursor.GetValueToString('FIGURE') );
...
procedure TAndroidModule1.jTextView2Click(Sender: TObject);
begin
