...
//start: 2013-october-26: a modified version of the simonsayz's "And_Controls"
//author: Jose Marques Pessoa jmpessoa at hotmail_com
//ver_0.1: 08-december-2013
//ver_0.2: 07-january-2014 (Add support for Android Api > 13)
...
type
...

 {
  ...
  public
    ...
    property  JavaObj : jObject           read FjObject;
    property Initialized : boolean read FInitialized;
    property App: jApp read refApp write refApp;
  end;
  ...
  // Activity Event
  Function  Java_Event_pAppOnScreenStyle         (env: PJNIEnv; this: jobject; context: JObject): integer;
  Procedure Java_Event_pAppOnNewIntent           (env: PJNIEnv; this: jobject; context: JObject);
  Procedure Java_Event_pAppOnDestroy             (env: PJNIEnv; this: jobject; context: JObject);
  Procedure Java_Event_pAppOnPause               (env: PJNIEnv; this: jobject; context: JObject);
  Procedure Java_Event_pAppOnRestart             (env: PJNIEnv; this: jobject; context: JObject);
  Procedure Java_Event_pAppOnResume              (env: PJNIEnv; this: jobject; context: JObject);
  Procedure Java_Event_pAppOnActive              (env: PJNIEnv; this: jobject; context: JObject);
  Procedure Java_Event_pAppOnStop                (env: PJNIEnv; this: jobject; context: JObject);
  Procedure Java_Event_pAppOnBackPressed         (env: PJNIEnv; this: jobject; context: JObject);
  Function  Java_Event_pAppOnRotate              (env: PJNIEnv; this: jobject; context: JObject; rotate : Integer) : integer;
  Procedure Java_Event_pAppOnConfigurationChanged(env: PJNIEnv; this: jobject; context: JObject);
  Procedure Java_Event_pAppOnActivityResult      (env: PJNIEnv; this: jobject; context: JObject; requestCode,resultCode : Integer; jData : jObject);
  ...
  // Form Event
  Procedure Java_Event_pOnClose                  (env: PJNIEnv; this: jobject; context: JObject);
...
begin
  ...
  case filePath of
      fpathNone: Result:='';
      fpathExt: Result:= gjApp.Path.Ext;
      fpathData: Result:= gjApp.Path.Dat;
      fpathDCIM: Result:= gjApp.Path.DCIM;
      fpathApp: Result:= gjApp.Path.App;
  end;
end;
...
Function stringLen(str: String): Integer;
 begin
  result := jStr_GetLength(gjApp.Jni.jEnv, gjApp.Jni.jThis, str);
 end;

// LORDMAN - 2013-07-30
Function getDateTime: String;
 begin
  result := jStr_GetDateTime(gjApp.Jni.jEnv, gjApp.Jni.jThis);
 end;

Procedure ShowMessage(msg: string);
begin
  if gjApp = nil then Exit;
  if not gjApp.Initialized then Exit;
  jToast(gjApp.Jni.jEnv, gjApp.Jni.jThis, msg);
end;
...
Function  Asset_SaveToFile(srcFile, outFile : String; SkipExists : Boolean = False) : Boolean;
 begin
  ...
  jAsset_SaveToFile(gjApp.Jni.jEnv,gjApp.Jni.jThis,srcFile,outFile);
...
Function  Asset_SaveToFileP(srcFile, outFile : string; SkipExists : Boolean = False) : Boolean;
 ...
 begin
  ...
  If ZipExtract(gjApp.Path.App,srcFile,Stream) then
...
begin
 { gjApp.Jni.jEnv:= env;
  gjApp.Jni.jThis:= this;
  gjApp.Jni.jActivity:= context;}
  case gjApp.Screen.Style of
    ssSensor    : Result := 0;
    ssPortrait  : Result := 1;
...
// by jmpessoa  - "App.java"  -->Main Activity: onStart()
Procedure Java_Event_pAppOnActive(env: PJNIEnv; this: jObject; context: JObject);
var
 Form: jForm;
begin
 ...
 Form := gjApp.Forms.Stack[gjApp.Forms.Index-1].Form;
...
begin
  ...
  //----update "App" global: to whom it may concern [ex: ShowMessage]--
  gjApp.Jni.jEnv:= env;
  gjApp.Jni.jThis:= this;
  gjApp.Jni.jActivity:= context;
  //------------------------------------------------------------------

  AForm := gjApp.Forms.Stack[gjApp.Forms.Index-1].Form;
  if not(Assigned(AForm)) then Exit;
  if AForm.FormState <> fsFormWork then Exit;

  if Assigned(AForm.OnBackButton) then AForm.OnBackButton(AForm);

  //------update JNI reference for jForm
  AForm.App.Jni.jEnv:= env;
  AForm.App.Jni.jThis:= this;
...
Function Java_Event_pAppOnRotate(env: PJNIEnv; this: jobject; context: JObject;
...
Procedure Java_Event_pAppOnActivityResult(env: PJNIEnv; this: jobject; context: JObject;
...
Procedure jApp.JNIEnvRefresh;
begin
  jApp_GetJNIEnv(Self.Jni.jEnv);
end;
...
Procedure jForm.Show;
 begin
  ...
  Self.App.Jni.jEnv:= gjApp.Jni.jEnv;
  Self.App.Jni.jEnv:= gjApp.Jni.jEnv;
...
begin
  ...
  begin
    CanClose := True;  //change by jmpessoa
...
begin
  ...
  If gjApp.Forms.Index > 0 then Dec(gjApp.Forms.Index);

  // LORDMAN - 2013-08-01 / Call Back - 현재 Form 이전것을 한다.

  Inx := gjApp.Forms.Index;
  if Assigned(gjApp.Forms.Stack[Inx].CloseCB.Event) then
  begin
    gjApp.Forms.Stack[Inx].CloseCB.Event(gjApp.Forms.Stack[Inx].CloseCB.Sender);
    gjApp.Forms.Stack[Inx].CloseCB.Event  := nil;
    gjApp.Forms.Stack[Inx].CloseCB.Sender := nil;
  end;

  if Form.MainActivity then  // App "The End"
  begin
     Form.App.Jni.jEnv:= env;
     Form.App.Jni.jThis:= this;
     Form.Free;
     gjApp.Finish;
  end
  else if Assigned(Form.OnClose) then Form.OnClose(Form);
...
begin
  Result:= FjRLayout; //jForm_Getlayout2(App.Jni.jEnv, App.Jni.jThis, FjObject);  {view}
end;
...
begin
  ...
  *)
  ...
  //by jmpessoa
  //jTextView_SetVisible(App.Jni.jEnv, App.Jni.jThis, FjObject, FVisible);
